...
export function OrderCard({ order, destination }: { order: DeliveryOrder; destination: string }) {
  ...
  return <button className={`order-card ${selected ? 'is-selected' : ''} ${blocked ? 'is-blocked' : ''}`} onClick={() => selectOrder(order.id)} type="button">
    ...
    <div className="order-card__metrics"><span><Icon name="box" size={14}/>{order.weightKg} kg</span><span>+{order.rewardCredits} CR</span>{order.deadlineMinute !== null ? <span><Icon name="clock" size={14}/>{Math.max(0, Math.round(order.deadlineMinute))}m</span> : null}</div>
