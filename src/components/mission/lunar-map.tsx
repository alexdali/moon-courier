...
export function LunarMap() {
  ...
  return <section className="map-shell">
    <div className="map-toolbar"><span>{t('TACTICAL MAP')}</span><span className="map-legend"><i className="legend-dot legend-dot--cyan"/>{t('selected route')} <i className="legend-dot legend-dot--red"/>{t('high risk')}</span></div>
    <svg className="lunar-map" viewBox="0 0 100 100" role="img" aria-label={t('Lunar delivery map')}>
      <defs>
        <radialGradient id="moonSurface"><stop offset="0" stopColor="#172631"/><stop offset="1" stopColor="#09131b"/></radialGradient>
      ...
      <rect width="100" height="100" fill="url(#moonSurface)"/><rect width="100" height="100" fill="url(#grid)"/>
      <g opacity=".24" fill="none" stroke="#6f7c83" strokeWidth=".35"><circle cx="18" cy="18" r="8"/><circle cx="18" cy="18" r="4.4"/><circle cx="72" cy="73" r="12"/><circle cx="72" cy="73" r="7"/><circle cx="48" cy="26" r="5"/><path d="M5 82c18-8 31-1 44-7s25-14 46-8"/></g>
      <g>{world.zones.map((zone) => <polygon key={zone.id} points={zone.polygon.map((point) => `${point.x},${point.y}`).join(' ')} fill={zone.color} fillOpacity={Math.min(.18, .055 * zone.riskMultiplier)} stroke={zone.color} strokeOpacity=".32" strokeWidth=".35" strokeDasharray="1.4 1.4"/>)}</g>
      <g>{world.edges.map((edge) => {
        ...
        return <line key={edge.id} x1={from.x} y1={from.y} x2={to.x} y2={to.y} className={active ? 'map-edge map-edge--active' : edge.baseRisk > .15 ? 'map-edge map-edge--risk' : 'map-edge'}/>;
      })}</g>
      ...
      <g>{orders.filter((order) => ['pending', 'assigned', 'in_transit'].includes(order.status)).map((order) => {
        ...
        return <g key={order.id} transform={`translate(${node.x + 2.8 + offsetX} ${node.y + offsetY})`} className={`order-marker ${selected ? 'is-selected' : ''} ${order.impossibleReason ? 'is-blocked' : ''}`}><rect x="-1.7" y="-1.7" width="3.4" height="3.4" transform="rotate(45)"/><text x={labelOnLeft ? -3.5 : 3.5} y="1" textAnchor={labelOnLeft ? 'end' : 'start'}>{order.code}</text></g>;
      })}</g>
      <g>{rovers.map((rover) => {
    ...
    <div className="map-footer"><span>{world.nodes.length} {t('sites')} · {world.edges.length} {t('routes')}</span><span>{t('Seed')} {missionSeed}</span></div>
  </section>;
}
