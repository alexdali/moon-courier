...
export function DeliveryResult() {
  ...
  return <aside className={`delivery-result delivery-result--${success ? 'success' : 'failure'}`}>
    ...
    <div><small>{t(success ? 'DELIVERY COMPLETE' : 'DELIVERY FAILED')}</small><h3>{replay.order.code} · {t(replay.order.title)}</h3><p>{success ? `${replay.rover.code} ${t('reached the destination.')}` : `${t('Failure')}: ${t(replay.delivery.failureCode ?? 'unknown incident')}.`}</p></div>
    <div className="delivery-result__metrics"><span><small>{t('Net result')}</small><strong>{(replay.delivery.actualNetCredits ?? 0) > 0 ? '+' : ''}{replay.delivery.actualNetCredits?.toFixed(0)} {t('CR')}</strong></span><span><small>{t('Battery')}</small><strong>{replay.rover.batteryPercent.toFixed(0)}%</strong></span><span><small>{t('Events')}</small><strong>{replay.events.length}</strong></span></div>
  </aside>;
}
