...
export function MissionToolbar() {
  ...
  return <div className="mission-toolbar">
    ...
    <div className="mission-toolbar__actions"><div className="view-switcher" role="group" aria-label={t('Interface version')}><button type="button" className={mode === 'simple' ? 'is-active' : ''} aria-pressed={mode === 'simple'} onClick={() => setMode('simple')}>{t('Simple')}</button><button type="button" className={mode === 'detailed' ? 'is-active' : ''} aria-pressed={mode === 'detailed'} onClick={() => setMode('detailed')}>{t('Detailed')}</button></div><div className="theme-switcher" role="group" aria-label={t('Color theme')}><button type="button" className={theme === 'light' ? 'is-active' : ''} aria-pressed={theme === 'light'} onClick={() => setTheme('light')}>{t('Light')}</button><button type="button" className={theme === 'dark' ? 'is-active' : ''} aria-pressed={theme === 'dark'} onClick={() => setTheme('dark')}>{t('Dark')}</button></div><LocaleSwitcher/><Link href="/analytics" className="icon-button" title={t('Mission debrief')}><Icon name="chart"/></Link><button className="icon-button" type="button" title={t('Reset demo')} onClick={() => void reset()} disabled={busy === 'reset'}><Icon name="reset"/></button></div>
  </div>;
}
