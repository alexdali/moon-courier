...
export function AnalyticsDashboard({ initial }: { initial: AnalyticsDashboardDto }) {
  ...
  return <div className="analytics-layout">
    <div className="kpi-grid">
      <article><span>{t('Total credits')}</span><strong>{data.kpis.credits.toFixed(0)} CR</strong><small className={data.kpis.netChange >= 0 ? 'text-mint' : 'text-red'}>{data.kpis.netChange >= 0 ? '+' : ''}{data.kpis.netChange.toFixed(0)} {t('from start')}</small></article>
