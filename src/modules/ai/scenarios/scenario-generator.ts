...
import { scenarioArchitectSystemPrompt, SCENARIO_ARCHITECT_PROMPT_VERSION } from '@/modules/ai/prompts/scenario-architect';
import { scenarioBlueprintJsonSchema, scenarioBlueprintSchema } from '@/modules/ai/schemas/scenario-blueprint-schema';
...
export class AiScenarioGenerator {
  constructor(private readonly env: AppEnv, private readonly client: OpenRouterClient) {}
  get promptVersion(): string { return SCENARIO_ARCHITECT_PROMPT_VERSION; }
  ...
  async run(input: {
    ...
    try {
      ...
      if (!validation.valid) throw new AiOutputError('Generated scenario failed deterministic validation', validation);
      const balance = analyzeScenarioBalance(scenario, 80);
      if (!balance.survivable) throw new AiOutputError('Generated scenario is not survivable in balance simulation', balance);
      return {
