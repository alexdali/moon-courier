...
# 19. Главная формулировка защиты

> Я построил не чат поверх игры, а небольшую систему поддержки операционных решений. Доменный движок рассчитывает маршруты, ограничения, риск и экономику. AI понимает цель пользователя, вызывает проверяемые инструменты и объясняет результат. DeepSeek используется как основная дешёвая модель, Luna — как явный fallback. Каждый результат сохраняется и воспроизводится.
